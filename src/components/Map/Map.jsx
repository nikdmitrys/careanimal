...
let Map = (props) => {
    ...
    return(
        <Container width={props.width} height={props.height}>
            <Search placeholder={props.placeholder} style={props.searchStyle} Selected={(place)=>{PlaceSelected(place)}} />
            <GoogleMap
            ...
            {!!props.stray &&
                props.stray.map((s, id)=>{
                    return(<Marker key={id} position={{lat: s.latlng[0], lng: s.latlng[1]}} onClick={(place)=>{setSelected(s); setPosition({lat: s.latlng[0], lng: s.latlng[1]})}} 
                    icon={{
            ...
            {!!props.house &&
                ...
                { selected &&
                <InfoWindow position={{lat:selected.latlng[0], lng:selected.latlng[1]}} onCloseClick={()=>{setSelected(null)}}>
                    <div>
                        <Img src={test} alt="" />
                    </div>
                </InfoWindow>
